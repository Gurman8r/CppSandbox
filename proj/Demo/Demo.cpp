...
namespace demo
{
	bool onProgramEnter(const ProgramEnterEvent & ev)
	{
		// Start Master Timer
		ML_Time.start();
	...
	bool onLoadResources(const LoadResourcesEvent & ev)
	{
	...
	void onDraw(const DrawEvent & ev)
	{
		ev.window.clear(ml::Color::Violet);
		{
			...
			if (ml::Shader & shader = shaders[GL_text])
			{
				...
				const uint32_t  fontSize = 32;
				const ml::vec2f offset = { 0.0f, -(float)fontSize };
				const ml::vec2f origin = { (float)fontSize, (float)ev.window.height() };
				const ml::vec4f colors[MAX_FONT] = {
					ml::Color::Red,
					ml::Color::Green,
					ml::Color::Blue,
					ml::Color::White,
