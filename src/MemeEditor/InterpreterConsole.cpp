...
namespace ml
{
	...
	void    InterpreterConsole::ClearLog()
	{
	...
	void    InterpreterConsole::AddLog(ml::CString fmt, ...)
	{
		// FIXME-OPT
		char buf[1024];
		va_list args;
	...
	void    InterpreterConsole::Draw(ml::CString title, bool* p_open)
	{
	...
	void    InterpreterConsole::ExecCommand(ml::CString command_line)
	{
		AddLog("# %s\n", command_line);
	...
	int		InterpreterConsole::TextEditCallbackStub(ImGuiInputTextCallbackData* data) // In C++11 you are better off using lambdas for this sort of forwarding callbacks
	{
		InterpreterConsole* console = (InterpreterConsole*)data->UserData;
		return console->TextEditCallback(data);
	}

	int     InterpreterConsole::TextEditCallback(ImGuiInputTextCallbackData* data)
	{
